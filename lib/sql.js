...
var sql = module.exports = {
    ...
    parse: function(sql, options, params) {
        var query = sql,
            values = params,
            i = 0,
            l, j, limit, newSql, found;

        if (!(sql instanceof Array)) {
            query = [sql];
        }

        if (values && !(values[0] instanceof Array)) {
            values = [params];
        }
        ...
        for (; i < l; i++) {
            newSql = query[i].split('LIMIT')[0];
            limit = 'SELECT';
            found = query[i].match(/\$\d+/g);
            
            for (j = 0; found && j < found.length; j++) {
                newSql = newSql.replace(found[j], Firebird.escape(values[i][j]));
            }
            ...
            if (options) {
                ...
                if (options.skip && !isNaN(options.skip)) {
                    limit += ' SKIP ' + options.skip;
                }
            }

            query[i] = newSql.replace('SELECT', limit).replace(/ILIKE/g, 'LIKE');
        }

        return query;
    },

    escapeParams: function(query) {
