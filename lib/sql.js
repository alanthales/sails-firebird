...
var sql = module.exports = {
    ...
    preflight: function(sql, options) {
        var query = sql.replace(/\$\d+/g, '?').split('LIMIT'),
            newStr = '';

        if (options.limit && !isNaN(options.limit)) {
            newStr += ' FIRST ' + options.limit;
        }

        if (options.skip && !isNaN(options.skip)) {
            newStr += ' SKIP ' + options.skip;
        }

        return query[0].replace('SELECT', ['SELECT', newStr].join(''));
    }
};
