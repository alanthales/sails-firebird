    ...
    var adapter = {
        ...
        create: function (connectionName, collectionName, data, cb, connection) {
            ...
            function __CREATE__(connection, cb) {
                ...
                function __INSERT__() {
                    ...
                    connection.query(sql.parse(_query.query, null, _query.values), function (err, result) {
                        if (err) return cb(handleQueryError(err));
            ...
            function __CREATE_EACH_ITEM__(connection, cb) {
                ...
                async.eachSeries(valuesList,
                    ...
                    function (data, cb) {
                        ...
                        connection.query(sql.parse(_query.query, null, _query.values), function (err, results) {
                            if (err) return cb(handleQueryError(err));
                    ...
                    $populateBuffers: function populateBuffers(options, next) {
                        ...
                        async.auto({
                                ...
                                processParent: function (next) {
                                    log('Firebird.populateBuffers: ', _query);

                                    client.query(sql.parse(_query.query[0], instructions, _query.values)[0], function __FIND__(err, result) {
                                        if (err) {
                                            return next(err);
                                        }
                                        ...
                                        var splitChildren = function (parent, next) {
                                            ...
                                            _.keys(parent).forEach(function (key) {
                                                ...
                                                if (!_.has(cache, split[0])) cache[split[0]] = {};
                                            ...
                                            if (_.keys(cache).length > 0) {
                                                _.keys(cache).forEach(function (pop) {
                                                    if (!_.has(cachedChildren, pop)) cachedChildren[pop] = [];
                                ...
                                processChildren: ['buildChildBuffers', function (next, results) {
                                    ...
                                    async.each(_query.query, function (q, next) {
                                        ...
                                        qs = qs.slice(0, -11);
                                        qs = qs.replace(/ORDER BY \D+ (ASC|DESC)/g, '');
                                        qs = qs.replace(/LIMIT [0-9]*/g, '');
        ...
        find: function (connectionName, collectionName, options, cb, connection) {
            ...
            function __FIND__(connection, cb) {
                ...
                connection.query(sql.parse(_query.query, options, _query.values)[0], function (err, result) {
        ...
        update: function (connectionName, collectionName, options, values, cb, connection) {
            ...
            function __UPDATE__(connection, cb) {
                ...
                connection.query(sql.parse(_query.query[0], options, _query.values)[0], function (err, results) {
                    if (err) return cb(err);
                    ...
                    // Run query
                    _query = sql.parse(_query.query, options, _query.values);
                    log('Firebird.update: ', _query);
                    ...
                    connection.query(_query, function (err, result) {
                        ...
                        connection.query(sql.parse(_query.query, criteria, _query.values)[0], function (err, result) {
                            if (err) return cb(err);
                            cb(null, result);
                        });
                    });
            ...
            function __DESTROY__(connection, cb) {
                ...
                async.auto({
                        ...
                        destroyRecords: ['findRecords', function (next) {
                            ...
                            connection.query(sql.parse(_query.query, null, _query.values), next);
                        }]
                    },
