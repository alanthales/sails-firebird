    ...
    var adapter = {
                    ...
                    $populateBuffers: function populateBuffers(options, next) {
                        ...
                        async.auto({
                                ...
                                processParent: function (next) {
                                    ...
                                    client.query(sql.preflight(_query.query[0], instructions), function __FIND__(err, result) {
                                        if (err) {
                                            log('Firebird.populateBuffers ERROR', err);
                                            return next(err);
                                        }

                                        log('Firebird.populateBuffers RESULTS', result);
                                        ...
                                        async.eachSeries(parentRecords, splitChildren, function (err) {
                                            if (err) {
                                                log('Async.eachSeries ERROR', err);
                                                return next(err);
                                            }
        ...
        find: function (connectionName, collectionName, options, cb, connection) {
            ...
            function __FIND__(connection, cb) {
                ...
                connection.query(sql.preflight(_query.query[0], options), function (err, result) {
