    ...
    var adapter = {
                    ...
                    $populateBuffers: function populateBuffers(options, next) {
                        ...
                        async.auto({
                                ...
                                processParent: function (next) {
                                    ...
                                    client.query(sql.parse(_query.query, instructions)[0], _query.values[0], function __FIND__(err, result) {
                                        if (err) {
                                            return next(err);
                                        }
        ...
        find: function (connectionName, collectionName, options, cb, connection) {
            ...
            function __FIND__(connection, cb) {
                ...
                connection.query(sql.parse(_query.query, options)[0], _query.values[0], function (err, result) {
        ...
        update: function (connectionName, collectionName, options, values, cb, connection) {
            ...
            function __UPDATE__(connection, cb) {
                ...
                connection.query(sql.parse(_query.query, options)[0], sql.values[0], function (err, results) {
                    if (err) return cb(err);
                    ...
                    connection.query(sql.parse(_query.query), _query.values, function (err, result) {
                        ...
                        connection.query(sql.parse(_query.query, criteria)[0], _query.values[0], function (err, result) {
                            if (err) return cb(err);
                            cb(null, result);
                        });
                    });
